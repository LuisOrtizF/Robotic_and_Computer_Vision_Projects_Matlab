...
static emlrtRSInfo gf_emlrtRSI = { 58, "power",
  "C:\\Program Files\\MATLAB\\R2016a\\toolbox\\eml\\lib\\matlab\\ops\\power.m" };

static emlrtRSInfo if_emlrtRSI = { 132, "applyScalarFunction",
...
void b_power(const emlrtStack *sp, const emxArray_real_T *a, emxArray_real_T *y)
{
  ...
  if (dimagree(y, a)) {
    ...
    emlrtErrorWithMessageIdR2012b(&c_st, &rc_emlrtRTEI, "MATLAB:dimagree", 0);
  }
...
void power(const real_T a[921600], real_T y[921600])
{
  int32_T k;
  for (k = 0; k < 921600; k++) {
    y[k] = a[k] * a[k];
